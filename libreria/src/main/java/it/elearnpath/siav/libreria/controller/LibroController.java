...
public class LibroController {
        ...
        @ApiOperation(value = "Ricerca tutti i libri scritti dall'autore inserito nel path tramite id", response = LibroDTO.class, produces = "application/json")
        @ApiResponses(value = { @ApiResponse(code = 200, message = "Tutto bene"),
                        @ApiResponse(code = 400, message = "Errore generico") })
        @GetMapping(value = "/search/autore/{autore}")
        public ResponseEntity<List<LibroDTO>> searchAllByAutore(@PathVariable("autore") @Valid Integer autore)
                        throws NotFoundException {
        ...
        public ResponseEntity<LibroDTO> insertLibro(@RequestBody @Valid LibroDTO libroDTO, BindingResult bindingResult)
                ...
                if (bindingResult.hasErrors()) {
                        String errMsg = errMessage.getMessage(bindingResult.getFieldError(),
